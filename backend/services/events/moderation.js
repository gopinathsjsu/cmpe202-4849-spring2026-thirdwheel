...
function buildPipeline(deps) {
    // Order matters: SpamFilter → CapacitySanity → admin queue (Handler base).
    // TrustedOrganizer auto-approve is intentionally disabled — every legitimate
    // event must pass through the admin moderation queue. The class is kept for
    // reference; re-enable by inserting it into the chain.
