...
const CategoryRepository = require('../../shared/repositories/CategoryRepository');
const { generateCalendarFile } = require('../../shared/utils/calendar');
...
router.post('/:id/cancel', authenticateToken, requireRole('organizer', 'admin'), asyncHandler(async (req, res) => {
    ...
    res.json({ message: 'Event cancelled. Attendees notified.', ...result });
}));

router.post('/:id/reschedule', authenticateToken, requireRole('organizer', 'admin'), validate({
    date: { required: true, type: 'string' },
    time: { required: true, type: 'string' },
}), asyncHandler(async (req, res) => {
    ...
    res.json({ message: 'Event rescheduled. Attendees notified.', ...result });
}));
